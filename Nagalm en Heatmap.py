...
# Een lus wordt gebruikt om elk gegevenspunt in de lijst gemdata te doorlopen.
    ...
    # Dit gebeurt ook pas na een kwart seconde om ruis te vermijden.
        if i >= kwart_seconde_samples * 2 and teind == 0:
            ...
            # Dit wordt gedaan met een kleine marge van 0.1 dB (tussen 13 en 13.1) om kleine variaties in de gemeten intensiteit te accepteren.
            # Dit helpt om het einde van de nagalm betrouwbaar te detecteren, zelfs bij kleine fluctuaties in de geluidsintensiteit.
    ...
    if audio_bestand.endswith(".wav"):
       ...
       # Als we 5 audiobestanden hebben verwerkt, berekenen we het gemiddelde van de nagalmtijden van die groep.
        if len(gemiddelde_nagalmtijden) % 5 == 0:
            # Controleer of het aantal elementen deelbaar is door 5
            # Dit betekent dat er een groep van 5 elementen is
            gemiddelde = sum(gemiddelde_nagalmtijden[-5:]) / 5 # Bereken het gemiddelde van de laatste 5 elementen
            print(f"Gemiddelde nagalmtijd van groep {len(gemiddelde_nagalmtijden)//5}: {gemiddelde}") # Druk het berekende gemiddelde af en geef aan tot welke groep het behoort
